...
function SafeInstance:DescendantWhichIsA(Class: string)
	return self:IfInstance(function()
...
function SafeInstance:IfInstance(Callback: (SafeInstance) -> SafeInstance | any): SafeInstance
	if typeof(self.Instance) == "Instance" then
		local ReturnValue = Callback(self)

		assert(
			typeof(ReturnValue) == "table" and ReturnValue._Type == "SafeInstance",
			"IfInstance callback did not return a SafeInstance object"
		)
